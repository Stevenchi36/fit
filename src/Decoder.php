...
class Decoder
{
    ...
    /**
     * Reads the file/stream and calls the provided callback for each decoded message.
     *
     * @param string|resource $fileOrStream
     * @param callable $messageHandler  Callback function that is called for each decoded message. The decoded message is passed as parameter to the callback.
     * @throws Exception 
     */
    public function stream(mixed $fileOrStream, callable $messageHandler): void
    {
        $this->decode($fileOrStream, $messageHandler);
    }

    private function decode(mixed $fileOrStream, callable $messageHandler): void
    {
        $handle = $fileOrStream;
        ...
        try {
            $this->readMessages(new IOReader($handle), $messageHandler);
    ...
    private function readMessages(IOReader $reader, callable $messageHandler): void
    {
        ...
        while ($reader->getOffset() - $header->getHeaderSize() < $header->getDataSize()) {
            ...
            switch ($recordHeader['message_type']) {
                ...
                case self::MESSAGE_TYPE_DEFINITION:
                    // definition message
                    $messageTypeDefinitions[$localMessagType] = $this->nextRecordDefinition($recordHeader['has_developer_data'], $reader);
                    break;
                ...
                case self::MESSAGE_TYPE_DATA:
                    ...
                    $num = $message->getGlobalMessageNumber();
                    if ($num === MesgNum::DEVELOPER_DATA_ID || $num === MesgNum::FIELD_DESCRIPTION) {
                        // store developer data id and field description messages to be able to assign developer fields later on
                        $devMessages->addMessage($message);
                    }
                    $messageHandler($message);
                    break;
